...
function connection() {
    ...
    xhr.onloadstart = function() {
        document.getElementById("characterSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-primary">Carregando personagem...</strong>' +
            '<div class="text-primary spinner-border ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    xhr.onerror = function() {
        document.getElementById("characterSection").innerHTML = '<h2 id="characterMainTitle">Um erro não esperado.</h2>';
    }
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            if (results["data"].count === 0) {
                document.getElementById("characterSection").innerHTML =
                    '<h2 id="characterMainTitle"><span style="font-weight:bold;">Nenhum resultado encontrado ' +
                ...
                document.getElementById("characterSection").innerHTML =
                    ...
                    "Pode ter ocorrido um erro do nosso lado, desculpe. <br> Neste caso, tente novamente mais tarde.</h2>";
...
function comics(characterID) {
    ...
    xhr.onerror = function() {
        document.getElementById("characterSection").innerHTML =
            '<h2 id="characterMainTitle">Ocorreu um erro, verifique a conexão.</h2>';
    ...
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            if (results["data"] == 0) {
                ...
                comicSection.innerHTML = "<h2>Sem quadrinhos disponíveis</h2>";
            } else {
                // comics available
                let output = "",
                    creators = "";
            ...
            document.getElementById("characterSection").innerHTML =
                '<h2 id="characterMainTitle">Pedido não recebido</h2>';
            document.getElementById("comicSection").innerHTML =
                '<h2 id="characterMainTitle">Pedido não recebido</h2>';
        }
    }
    xhr.onloadend = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-success">Sucesso em nosso Objetivo</strong>';
    }
    xhr.send()
}
...
function singleComic() {
    ...
    xhr.onloadstart = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Carregando informações de quadrinhos ...</strong>' +
    ...
    xhr.onloadend = function() {
        document.getElementById("comicsSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Sucesso em nosso Objetivo</strong>';
    }
...
function comicCreator() {
    const urlQueryParameters = new URLSearchParams(window.location.search), // http://localhost:8080/creator.php?creator-id=8635&name=alexsoarespaes
    ...
    xhr.onloadstart = function() {
        document.getElementById("comicCreatorSpinnerSection").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Carregando informações do criador ...</strong>' +
            '<div class="spinner-border text-secondary ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    ...
    xhr.onerror = function() {
        comicCreatorContainerDiv.innerHTML =
            '<h2 id="header-main-title single-comic__main-title">Ocorreu um erro, verifique a conexão.</h2>';
    }
    ...
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            output +=
                '<h1 class="header-main-title single-comic__main-title">Criador</h1>' +
                '<div class="card mb-3">' +
                '<div class="row no-gutters">' +
                '<div class="col-md-4">' +
                '<img src="' +
            ...
            output +=
                '<h1 class="header-main-title single-comic__main-title">Histórias em quadrinhos</h1>' +
            ...
            comicCreatorContainerDiv.innerHTML =
                '<h2 id="header-main-title single-comic__main-title">Ocorreu um erro, verifique a conexão ou pedido incorreto.</h2>';
        }
    }
...
function creatorSingleComic(comicResourceURI) {
    ...
    xhr.onloadstart = function() {
        document.getElementById("comicCreatorSpinnerSection1").innerHTML =
            '<strong id="spinnerText" class="text-secondary">Carregando quadrinhos ...</strong>' +
            '<div class="spinner-border text-secondary ml-auto" role="status" ' +
            'aria-hidden="true" id="spinner"></div>';
    }
    xhr.onerror = function() {
